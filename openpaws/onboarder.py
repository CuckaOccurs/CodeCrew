"""
OpenPaws Onboarder — Clean, local-first setup wizard.
...
class Onboarder:
    ...
    def run(self):
        ...
        self.console.print(Panel(
            "[bold #ffffff]Welcome to OpenPaws[/bold #ffffff]\n\n"
            "[dim #9a9996]OpenPaws is 100% free, open-source, and runs entirely on your machine. "
        ...
        if not Confirm.ask("\n[yellow]Shall we set things up?[/yellow]", default=True):
            self.console.print("[dim]No problem. You can run `openpaws onboard` anytime.[/dim]")
            return
    ...
    def step_gateway(self):
        ...
        self.console.print("[dim]Connect OpenPaws to Telegram, Discord, or Slack so you can chat with your AI from anywhere.[/dim]")
    ...
    def step_finish(self):
        ...
        config_dir = Path.home() / ".config" / "openpaws"
        ...
        self.console.print(Panel(
            "[dim #9a9996]💖 OpenPaws is free forever. If you ever want to support the project, "
            "donations are welcome but never required. We build this because it needs to exist.[/dim #9a9996]",
            border_style="#26a269"
        ))
        self.console.print("\n[bold #26a269]🐱 All set! Run `openpaws .` to start.[/bold #26a269]\n")
