...
Saved files land in ~/Projects/Airmy/conversations-db/ as chunked Markdown.
"""
...
SAVE_DIR = Path.home() / "Projects" / "Airmy" / "conversations-db"
...
def _save_chunks(lines: list[str], label: str) -> list[Path]:
    ...
    for i, start in enumerate(range(0, max(len(lines), 1), CHUNK_SIZE), start=1):
        ...
        path = SAVE_DIR / f"{label}-{stamp}{suffix}.md"
