...
class OllamaProvider(BaseProvider):
    ...
    def chat(self, messages: list[dict[str, Any]], tools: list[dict] | None = None) -> dict[str, Any]:
        ...
        try:
            # Shortened timeout for non-streaming to avoid infinite hangs
            resp = requests.post(
            ...
            return {"role": "assistant", "content": f"Error: Failed to connect to Ollama at {self.host} ({e})"}

        msg = data.get("message", {})
        content = msg.get("content", "")

        if not content and "tool_calls" not in msg:
            return {"role": "assistant", "content": "⚠️ Model returned empty response."}
        ...
        if "tool_calls" in msg and isinstance(msg["tool_calls"], list):
            ...
            for tc in msg["tool_calls"]:
                fn = tc.get("function", {})
                if isinstance(fn.get("arguments"), (str, dict)):
    ...
    def chat_stream(
        ...
        # Detect if we should use 'think' mode (DeepSeek-R1, etc)
        is_reasoner = any(kw in self.model.lower() for kw in ["r1", "thought", "thinker"])
        ...
        try:
            resp = requests.post(
                f"{self.host}/api/chat",
                json=payload,
                timeout=(10, None), # Connection timeout 10s, read timeout infinite
                stream=True,
            )
            resp.raise_for_status()
        ...
        # If both are empty, it stalled
        if not full_content.strip() and not tool_calls_raw:
             full_content = "⚠️ Model stalled or returned no content."
