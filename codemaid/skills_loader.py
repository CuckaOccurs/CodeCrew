...
def load_dict(name: str = "custom") -> dict:
...
def get_load_status(load_cfg: dict | None = None) -> dict:
    ...
    dict_files        = [DICT_DIR / f"{d}.yaml" for d in lc.get("dicts", [])]
...
def build_system_prompt(profile_name: str = "default", load_cfg: dict | None = None) -> str:
    ...
    # 5. Backbone (Skipped in Lean Mode)
    backbone = ""
    if not is_lean:
        backbone = (
            "## Operating Instructions (The MOP Backbone)\n"
