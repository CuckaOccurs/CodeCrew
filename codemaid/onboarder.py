...
from codemaid.defaults import (
    PROVIDER_DEFAULTS, PROVIDER_ENV_VARS,
    DEFAULT_PROVIDER, DEFAULT_MODEL,
    DEFAULT_OLLAMA_HOST, DEFAULT_OPENWEBUI_HOST
)

# Internal helper for UI display - combines model and env var
_PROVIDER_UI_METADATA = {
    p: (PROVIDER_DEFAULTS[p], PROVIDER_ENV_VARS.get(p))
...
class Onboarder:
    ...
    def step_provider(self) -> None:
        ...
        if provider == "openwebui":
            host = Prompt.ask("OpenWebUI host", default=DEFAULT_OPENWEBUI_HOST)
    ...
    def step_model(self) -> None:
        ...
        default_m = PROVIDER_DEFAULTS.get(provider, DEFAULT_MODEL)

        if provider == "ollama":
            # Try to list available models
            try:
                import requests
                host = self.config.get("host", DEFAULT_OLLAMA_HOST)
    ...
    def step_profiles(self) -> None:
        ...
        while adding:
            ...
            provider = Prompt.ask("Provider", default=self.config.get("provider", DEFAULT_PROVIDER))
            model    = Prompt.ask("Model",    default=PROVIDER_DEFAULTS.get(provider, DEFAULT_MODEL))
            api_key  = ""
            env_var = PROVIDER_ENV_VARS.get(provider)
            if env_var:
